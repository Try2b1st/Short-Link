package org.wgz.shortlink.gateway.filter;
...
import org.wgz.shortlink.gateway.config.FilterConfig;
import org.wgz.shortlink.gateway.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<FilterConfig> {
    ...
    public GatewayFilter apply(FilterConfig config) {
        return (exchange, chain) -> {
            ...
            String requestMethod = serverHttpRequest.getMethod().toString();

            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                // 不在白名单 或者 为修改用户信息 进行验证
    ...
    /**
     * 请求地址是否在白名单内 or 路径为 /api/short-link/admin/v1/user且请求方法不为POST
     *
     * @param requestPath   请求路径
     * @param requestMethod 请求方法
     * @param whitePathList 白名单
     * @return 在白名单 或者 路径为 /api/short-link/admin/v1/user且请求方法不为POST 放行 返回 true
     */
